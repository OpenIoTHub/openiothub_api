...
class GatewayLoginManager {
  static Future<LoginResponse> LoginServerByServerInfo(
      ServerInfo serverInfo, String gatewayIp, int gatewayPort) async {
    final channel = await Channel.getGateWayChannel(gatewayIp, gatewayPort);
    final stub = GatewayLoginManagerClient(channel);
    LoginResponse loginResponse =
        await stub.loginServerByServerInfo(serverInfo);
    print('LoginServerByServerInfo: ${loginResponse}');
    channel.shutdown();
    return loginResponse;
  }

  static Future<Token> GetOpenIoTHubToken(
